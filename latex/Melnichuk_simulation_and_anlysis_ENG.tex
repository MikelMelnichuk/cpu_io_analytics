...
На рис \ref{fig:naive_mu_prediction} можно увидеть кривую предсказания регрессионной модели(фиолетовым) и фактическое время измеренное на экспериментальном стенде (чёрным), а также можете увидеть метрики качества в таблице \ref{tab:polynomial_model_metrics}.
