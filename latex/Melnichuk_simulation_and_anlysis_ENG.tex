...
На рис \ref{fig:polynom_2_degree_prediction_compare} можно увидеть кривую предсказания регрессионной модели(фиолетовым) и фактическое время измеренное на экспериментальном стенде (чёрным), а также можете увидеть метрики качества в таблице \ref{tab:polynomial_model_metrics}.

\begin{figure}[!htbp]
  \centering
  \includegraphics[height=10cm]{polynom_2_degree_prediction_compare.png} \\
  \caption{Измеренное время обслуживания (Empirical) и предсказанная полиномиальной моделью, средний показатель качества по всем альфам описан в таблице \ref{tab:polynomial_model_metrics}}
  \label{fig:polynom_2_degree_prediction_compare}
\end{figure}

\begin{figure}[!htbp]
  \centering
  \includegraphics[height=10cm]{polynom_2_degree_prediction_compare_smooth.png} \\
  \caption{Измеренное время обслуживания (Empirical) и предсказанная полиномиальной моделью, средний показатель качества по всем альфам описан в таблице \ref{tab:polynomial_model_metrics}, для наглядности был применён savgol filter из библиотеки scipy на обе кривые}
  \label{fig:polynom_2_degree_prediction_compare_smooth}
\end{figure}

\begin{table}[htbp]
  \centering
  \caption{Model Performance Metrics per $\alpha$}
  \label{tab:polynomial_model_metrics}
